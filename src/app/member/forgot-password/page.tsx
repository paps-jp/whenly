import Link from "next/link";
import type { Metadata } from "next";
import { ForgotPasswordForm } from "@/components/ForgotPasswordForm";
import { requestMemberPasswordReset } from "@/lib/actions/password-reset";
import { getServerMessages } from "@/lib/i18n/server";

export async function generateMetadata(): Promise<Metadata> {
  const m = await getServerMessages();
  return { title: m.auth.forgotMetaTitle, robots: { index: false, follow: true } };
}

export default async function MemberForgotPasswordPage() {
  const m = await getServerMessages();

  return (
    <main className="mx-auto flex w-full max-w-md flex-1 flex-col justify-center px-4 py-16">
      <Link
        href="/member/login"
        className="mb-4 inline-flex items-center gap-1 text-sm font-medium text-slate-500 transition-colors hover:text-indigo-600"
      >
        {m.common.backToLogin}
      </Link>
      <h1 className="text-center text-2xl font-extrabold tracking-tight text-slate-900">
        {m.auth.forgotParticipantTitle}
      </h1>
      <p className="mt-2 text-center text-sm text-slate-500">{m.auth.forgotDescription}</p>
      <div className="mt-8 rounded-2xl border border-slate-100 bg-white p-6 shadow-sm shadow-slate-200/50">
        <ForgotPasswordForm action={requestMemberPasswordReset} />
      </div>
    </main>
  );
}
